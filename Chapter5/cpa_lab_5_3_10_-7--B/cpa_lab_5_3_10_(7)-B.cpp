...
bool Fraction::isEqual(Fraction that)
{
	this->Reduce();
	that.Reduce();
	if (that.denominator == this->denominator && this->numerator == that.numerator)
		return true;
	else
		return false;
}
...
int main(void) {
	...
	try {
		...
		if (fraction.isEqual(temp))
			k = '=';
		if (fraction.isGreaterThan(temp))
			k = '>';
		if (fraction.isLessThan(temp))
			k = '<';

		cout << fraction.toString() << " " << k << " " << temp.toString() << endl;
